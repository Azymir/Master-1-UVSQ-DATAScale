...
1. Énumérez une liste de cas de tests à réaliser.	
    ...
    1. il n'est pas possible de realiser une transaction d'un montant qui est inferieur a 0
    1. il n'est pas possible de realiser une transaction d'un montant qui est superieur au solde du compte
    1. L'utilisateur peut debiter son compte
    1. L'utilisateur peut crediter son compte
    1. L'utilisateur peut faire un virement vers un autre compte
...
1. Exécuter le programme. Que se passe-t-il ? De quel type d'erreur s'agit-il ? 
    * Renvoie le premier caractère répété. Un caractère est répété s’il se produit au moins deux fois dans les positions adjacentes.
...
1. Exécuter le programme. Que se passe-t-il ?
    * Renvoie le premier caractère qui se répète. Un caractère est multiple si elle se produit au moins deux fois dans le mot, pas nécessairement dans les positions adjacentes.
...
1. Exécuter le programme. Que se passe-t-il ?
    * Renvoie le nombre de groupes de caractères répétés.
...
Quel impact cela aura sur l'implémentation ?
    * Une classe immuable est une classe dont les instances sont immuables par conception et implémentation. Un objet immuable est un objet dont l'état ne peut pas être modifié. 
...
Quels [modificateurs](https://docs.oracle.com/javase/specs/jls/se11/html/jls-8.html#jls-8.3.1) doit-on leur appliquer ?
    * Il faut créer les objets numérateur et le dénominateur. 
